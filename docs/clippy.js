(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.clippy = factory(global.$));
    ...
    Queue.prototype._progressQueue = function _progressQueue () {
        ...
        // execute function
        var completeFunction = $__default["default"].proxy(this.next, this);
        f(completeFunction);
    };
    ...
    var Animator = function Animator (el, path, data, sounds) {
        ...
        for (var i = 1; i < this._data.overlayCount; i++) {
            var inner = this._setupElement($__default["default"]('<div></div>'));
            curr.append(inner);
    ...
    Animator.prototype._step = function _step () {
        ...
        this._loop = window.setTimeout($__default["default"].proxy(this._step, this), this._currentFrame.duration);
    ...
    Balloon.prototype._setup = function _setup () {
        ...
        this._balloon = $__default["default"]('<div class="clippy-balloon"><div class="clippy-tip"></div><div class="clippy-content"></div></div> ').hide();
        this._content = this._balloon.find('.clippy-content');

        $__default["default"](document.body).append(this._balloon);
    };
    ...
    Balloon.prototype._position = function _position (side) {
        ...
        o.top -= $__default["default"](window).scrollTop();
        o.left -= $__default["default"](window).scrollLeft();
    ...
    Balloon.prototype._isOut = function _isOut () {
        ...
        var wW = $__default["default"](window).width();
        var wH = $__default["default"](window).height();
        var sT = $__default["default"](document).scrollTop();
        var sL = $__default["default"](document).scrollLeft();
    ...
    Balloon.prototype.hide = function hide (fast) {
        ...
        this._hiding = window.setTimeout($__default["default"].proxy(this._finishHideBalloon, this), this.CLOSE_BALLOON_DELAY);
    };
    ...
    Balloon.prototype._sayWords = function _sayWords (text, hold, complete) {
        ...
        this._addWord = $__default["default"].proxy(function () {
            if (!this._active) { return; }
            if (idx > words.length) {
                ...
                idx++;
                this._loop = window.setTimeout($__default["default"].proxy(this._addWord, this), time);
            }
        }, this);
    ...
    var Agent = function Agent (path,data,sounds) {
        ...
        this._queue = new Queue($__default["default"].proxy(this._onQueueEmpty, this));

        this._el = $__default["default"]('<div class="clippy"></div>').hide();

        $__default["default"](document.body).append(this._el);
    ...
    Agent.prototype.moveTo = function moveTo (x, y, duration) {
        ...
        this._addToQueue(function (complete) {
            ...
            var callback = $__default["default"].proxy(function (name, state) {
                ...
                if (state === Animator.States.WAITING) {
                    this._el.animate({ top: y, left: x }, duration, $__default["default"].proxy(function () {
    ...
    Agent.prototype._playInternal = function _playInternal (animation, callback) {
        ...
        if (this._isIdleAnimation() && this._idleDfd && this._idleDfd.state() === 'pending') {
            this._idleDfd.done($__default["default"].proxy(function () {
    ...
    Agent.prototype.play = function play (animation, timeout, cb) {
        ...
        this._addToQueue(function (complete) {
            ...
            if (timeout) {
                window.setTimeout($__default["default"].proxy(function () {
                    if (completed) { return; }
                    // exit after timeout
                    this._animator.exitAnimation();
                }, this), timeout);
    ...
    Agent.prototype.show = function show (fast) {
        ...
        if (this._el.css('top') === 'auto' || !this._el.css('left') === 'auto') {
            var left = $__default["default"](window).width() * 0.8;
            var top = ($__default["default"](window).height() + $__default["default"](document).scrollTop()) * 0.8;
    ...
    Agent.prototype._onQueueEmpty = function _onQueueEmpty () {
        ...
        this._idleDfd = $__default["default"].Deferred();

        this._animator.showAnimation(idleAnim, $__default["default"].proxy(this._onIdleComplete, this));
    };
    ...
    Agent.prototype._setupEvents = function _setupEvents () {
        $__default["default"](window).on('resize', $__default["default"].proxy(this.reposition, this));

        this._el.on('mousedown', $__default["default"].proxy(this._onMouseDown, this));

        this._el.on('dblclick', $__default["default"].proxy(this._onDoubleClick, this));
    };
    ...
    Agent.prototype.reposition = function reposition () {
        ...
        var wW = $__default["default"](window).width();
        var wH = $__default["default"](window).height();
        var sT = $__default["default"](window).scrollTop();
        var sL = $__default["default"](window).scrollLeft();
    ...
    Agent.prototype._startDrag = function _startDrag (e) {
        ...
        this._moveHandle = $__default["default"].proxy(this._dragMove, this);
        this._upHandle = $__default["default"].proxy(this._finishDrag, this);

        $__default["default"](window).on('mousemove', this._moveHandle);
        $__default["default"](window).on('mouseup', this._upHandle);

        this._dragUpdateLoop = window.setTimeout($__default["default"].proxy(this._updateLocation, this), 10);
    };
    ...
    Agent.prototype._finishDrag = function _finishDrag () {
        ...
        // remove handles
        $__default["default"](window).off('mousemove', this._moveHandle);
        $__default["default"](window).off('mouseup', this._upHandle);
        // resume animations
        this._balloon.show();
        this.reposition();
        this.resume();
    ...
    Agent.prototype._addToQueue = function _addToQueue (func, scope) {
        if (scope) { func = $__default["default"].proxy(func, scope); }
    ...
    var load = function load (name, successCb, failCb, base_path) {
        ...
        $__default["default"].when(mapDfd, agentDfd, soundsDfd).done(cb).fail(failCb);
    };
    ...
    load._loadMap = function _loadMap (path) {
        ...
        dfd = load._maps[path] = $__default["default"].Deferred();
    ...
    load._loadSounds = function _loadSounds (name, path) {
        ...
        // set dfd if not defined
        dfd = load._sounds[name] = $__default["default"].Deferred();
    ...
    load._getAgentDfd = function _getAgentDfd (name) {
        var dfd = load._data[name];
        if (!dfd) {
            dfd = load._data[name] = $__default["default"].Deferred();
        }
        return dfd;
    };
